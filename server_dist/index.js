...
async function cleanupProcesses() {
  ...
  try {
    ...
    const entries = fs5.readdirSync("/tmp").filter(
      (d) => d.startsWith("wppconnect-") || d.startsWith("barmagly-")
    );
    for (const e of entries) {
      try {
        fs5.rmSync(`/tmp/${e}`, { recursive: true, force: true });
      } catch {
      }
    }
  } catch {
...
var whatsappService = {
  ...
  async connect() {
    ...
    try {
      ...
      if (envChrome && fs5.existsSync(envChrome)) {
      ...
      if (!browserPath) {
        try {
          ...
          if (nixFound && fs5.existsSync(nixFound)) {
      ...
      fs5.mkdirSync(CHROME_DATA_DIR, { recursive: true });
      fs5.mkdirSync(TOKEN_DIR, { recursive: true });
      const browserArgs = [
        "--no-sandbox",
  ...
  async sendText(phone, text2, _attempt = 1) {
    ...
    try {
      await client.sendText(toChatId(phone), text2);
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/license/validate", async (req, res) => {
    try {
      ...
      console.log("[VALIDATE] Incoming request details:", { licenseKey, email: email ? email.substring(0, 2) + "***" : void 0, deviceId });
      if (!licenseKey) {
...
`;
  ...
  if (!fs2.existsSync(TENANT_BACKUP_DIR)) fs2.mkdirSync(TENANT_BACKUP_DIR, { recursive: true });
  app2.get("/api/backup/list", async (req, res) => {
    try {
      const tenantId = req.tenantId;
      if (!tenantId) return res.status(401).json({ error: "Not authorized" });
      const files = fs2.readdirSync(TENANT_BACKUP_DIR).filter((f) => f.startsWith(`backup_tenant_${tenantId}_`) && f.endsWith(".json")).map((f) => {
        const stat = fs2.statSync(path.join(TENANT_BACKUP_DIR, f));
  ...
  app2.post("/api/backup/restore/:filename", async (req, res) => {
    try {
      ...
      const filepath = path.join(TENANT_BACKUP_DIR, filename);
      if (!fs2.existsSync(filepath)) return res.status(404).json({ error: "Backup not found" });
      const snapshot = JSON.parse(fs2.readFileSync(filepath, "utf-8"));
...
var BACKUP_DIR = path2.resolve(process.cwd(), "backups");
if (!fs3.existsSync(BACKUP_DIR)) fs3.mkdirSync(BACKUP_DIR, { recursive: true });
async function createTenantBackup(tenantId) {
  ...
  fs3.writeFileSync(filepath, JSON.stringify(snapshot, null, 2));
  console.log(`[BACKUP] Created ${filename} (${Math.round(fs3.statSync(filepath).size / 1024)}KB)`);
  return filename;
}
function pruneOldBackups() {
  try {
    const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1e3;
    fs3.readdirSync(BACKUP_DIR).forEach((f) => {
      const fp = path2.join(BACKUP_DIR, f);
      if (fs3.statSync(fp).mtimeMs < cutoff) fs3.unlinkSync(fp);
    });
  } catch (e) {
...
function registerSuperAdminRoutes(app2) {
  ...
  app2.get("/api/super-admin/backup/list", requireSuperAdmin, async (_req, res) => {
    try {
      const files = fs3.readdirSync(BACKUP_DIR).filter((f) => f.endsWith(".json")).map((f) => {
        const stat = fs3.statSync(path2.join(BACKUP_DIR, f));
        return { filename: f, size: stat.size, createdAt: stat.mtime.toISOString() };
      }).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      res.json(files);
    } catch (e) {
  ...
  app2.get("/api/super-admin/backup/download/:filename", requireSuperAdmin, async (req, res) => {
    try {
      ...
      res.send(fs3.readFileSync(filepath));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app2.post("/api/super-admin/backup/restore/:filename", requireSuperAdmin, async (req, res) => {
    try {
      const filename = path2.basename(req.params.filename);
      const filepath = path2.join(BACKUP_DIR, filename);
      if (!fs3.existsSync(filepath)) return res.status(404).json({ error: "Backup file not found" });
...
function isPublicRoute(path4) {
  if (PUBLIC_ROUTES.some((route) => path4.startsWith(route))) return true;
  if (PUBLIC_ROUTE_PATTERNS.some((pattern) => pattern.test(path4))) return true;
  return false;
}
var SEED_ROUTES = [
...
function isSeedRoute(path4) {
  return SEED_ROUTES.some((route) => path4.startsWith(route));
}
...
function configureExpoAndLanding(app2) {
  ...
  const dashboardPath = path3.resolve(
    process.cwd(),
    "server",
    "templates",
    "dashboard.html"
  ...
  <text x="256" y="330" text-anchor="middle" font-family="system-ui,-apple-system,sans-serif" font-size="240" font-weight="900" fill="url(#accent)">B</text>
    ...
    if (req.path === "/apple-touch-icon.png" || req.path === "/apple-touch-icon-precomposed.png") {
      const iconPath = path3.resolve(process.cwd(), "assets", "images", "icon.png");
      if (fs4.existsSync(iconPath)) {
        const iconBuf = fs4.readFileSync(iconPath);
...
`;
    ...
    if (req.path === "/dashboard") {
      const dbTemplate = fs4.readFileSync(dashboardPath, "utf-8");
      res.setHeader("Content-Type", "text/html; charset=utf-8");
    ...
    if (storeByIdMatch) {
      try {
        ...
        const storePath = path3.resolve(process.cwd(), "server", "templates", "restaurant-store.html");
        let html = fs4.readFileSync(storePath, "utf-8");
  ...
  app2.get("/{*splat}", (req, res, next) => {
    ...
    if (fs4.existsSync(staticIndexPath)) {
      let html = fs4.readFileSync(staticIndexPath, "utf-8");
      html = injectPWATags(html);
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      return res.status(200).send(html);
    }
